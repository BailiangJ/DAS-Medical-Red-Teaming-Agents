"""Specialist and orchestrator prompts for medical hallucination detection.

The prompts define the classification taxonomy, agent roles, output format, and
hallucination/accuracy subcategories used by the detector workflow.
"""
...
10. Return exactly one JSON object matching this shape:
   {"merged_codes": ["1", "3"], "rationale": "...", "agent_decisions": [...]}
   Use "0" or "0.5" for merged_codes when no root-code list applies. Do not add markdown or XML wrappers.
...
3. Compare that global ideal response to the actual LLM response:
  ...
  - **Output** the required JSON object with the 7 agent decisions.
